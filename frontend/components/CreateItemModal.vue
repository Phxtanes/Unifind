<template>
  <transition name="modal">
    <div v-if="show"
      class="fixed inset-0 z-50 overflow-y-auto bg-slate-900/60 flex items-center justify-center p-4 transition-all font-sans">
      <div
        class="bg-white rounded-3xl shadow-2xl border border-slate-100 overflow-hidden max-w-6xl w-full flex flex-col max-h-[90vh] modal-card">

        <!-- Header -->
        <div
          class="px-8 py-6 border-b border-slate-100 flex justify-between items-center bg-white shrink-0 select-none">
          <div class="flex items-center gap-4">
            <div
              class="w-12 h-12 rounded-2xl bg-indigo-600 flex items-center justify-center text-white shadow-lg shadow-indigo-600/20">
              <font-awesome :icon="['fas', 'briefcase']" class="text-lg" />
            </div>
            <div>
              <h2 class="text-base font-black text-slate-800 tracking-tight">{{ editItem ? $t('แก้ไขข้อมูลสิ่งของที่พบ')
                : $t('แจ้งสิ่งของที่พบ') }}</h2>
              <p class="text-slate-400 text-xs font-semibold mt-0.5">{{
                $t('กรุณากรอกข้อมูลให้ครบถ้วนเพื่อความรวดเร็วในการติดตาม') }}</p>
            </div>
          </div>
          <button @click="$emit('close')"
            class="w-8 h-8 rounded-full border border-slate-100 hover:bg-slate-50 flex items-center justify-center text-slate-400 hover:text-slate-700 transition duration-150">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div class="space-y-4">
                <div class="flex items-center gap-2 mb-4">
                  ...
                  <h3 class="text-xs font-extrabold text-blue-600 uppercase tracking-wider">{{ $t('ข้อมูลสิ่งของ') }}
                  </h3>
                </div>

                <!-- ชื่อสิ่งของ -->
                <div>
                  <label class="block text-xs font-bold text-slate-650 mb-1.5">{{ $t('ชื่อสิ่งของ') }} <span
                      class="text-red-500">*</span></label>
                  <div class="relative">
                    <input v-model="form.item_name" type="text" required
                      :placeholder="$t('เช่น กระเป๋าสตางค์, iPad Pro')"
                ...
                <div>
                  <div class="flex justify-between items-center mb-1.5">
                    <label class="block text-xs font-bold text-slate-650">{{ $t('หมวดหมู่สิ่งของ') }} <span
                        class="text-red-500">*</span></label>
                    <button type="button" @click="showAddCategory = true"
                      class="text-[10px] font-extrabold text-indigo-655 hover:text-indigo-850 transition flex items-center gap-1">
                ...
                <div>
                  <div class="flex justify-between items-center mb-1.5">
                    <label class="block text-xs font-bold text-slate-650">{{ $t('สถานที่พบ') }} <span
                        class="text-red-500">*</span></label>
                    <button type="button" @click="showAddLocation = true"
                      class="text-[10px] font-extrabold text-indigo-655 hover:text-indigo-850 transition flex items-center gap-1">
                  ...
                  <div class="relative">
                    <select v-model="form.location_id" required
                      class="w-full pl-4 pr-12 py-2.5 bg-slate-50/50 hover:bg-slate-50/85 focus:bg-white border border-slate-200 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-100 rounded-xl outline-none text-xs text-slate-700 font-semibold appearance-none transition">
                    ...
                    <div
                      class="pointer-events-none absolute inset-y-0 right-0 flex items-center px-3.5 text-slate-400 gap-1.5">
                ...
                <transition name="fade">
                  <div v-if="form.location_id === 'custom'" class="mt-3">
                    <label class="block text-xs font-bold text-slate-650 mb-1.5">{{ $t('ระบุสถานที่พบเอง') }} <span
                        class="text-red-500">*</span></label>
                    <div class="relative">
                      <input v-model="customLocationName" type="text" required
                        :placeholder="$t('ระบุสถานที่ เช่น อาคารเรียน 3 ห้อง 302')"
                        class="w-full pl-4 pr-10 py-2.5 bg-slate-50/50 hover:bg-slate-50/85 focus:bg-white border border-slate-200 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-100 rounded-xl outline-none text-xs text-slate-700 font-semibold transition" />
                      <div
                ...
                <!-- วันและเวลาที่พบ -->
                <div>
                  <label class="block text-xs font-bold text-slate-650 mb-1.5">{{ $t('วันและเวลาที่พบ') }} <span
                      class="text-red-500">*</span></label>
                  <div class="relative">
                    <input v-model="form.found_date" type="datetime-local" required @click="showDateTimePicker"
                ...
                <div>
                  ...
                  <textarea v-model="form.description" rows="3"
                    :placeholder="$t('ระบุตำหนิ สี สติกเกอร์ หรือจุดสังเกตเฉพาะ...')"
              ...
              <div class="space-y-4">
                <div class="flex items-center gap-2 mb-4">
                  <div
                    class="w-6 h-6 rounded-lg bg-emerald-50 flex items-center justify-center text-emerald-500 font-bold">
                    <font-awesome :icon="['fas', 'user']" class="text-[11px]" />
                  </div>
                  <h3 class="text-xs font-extrabold text-emerald-600 uppercase tracking-wider">{{ $t('ข้อมูลผู้พบส่ง')
                    }}</h3>
                </div>

                <!-- ชื่อผู้พบส่ง -->
                <div>
                  <label class="block text-xs font-bold text-slate-650 mb-1.5">{{ $t('ชื่อผู้พบส่ง') }} <span
                      class="text-red-500">*</span></label>
                  <div class="relative">
                ...
                <!-- ประเภทสถานะผู้พบ -->
                <div>
                  <label class="block text-xs font-bold text-slate-650 mb-1.5">{{ $t('ประเภทสถานะผู้พบ') }} <span
                      class="text-red-500">*</span></label>
                  <div class="relative">
                    <select v-model="form.finder_type" required
                      class="w-full pl-4 pr-10 py-2.5 bg-slate-50/50 hover:bg-slate-50/85 focus:bg-white border border-slate-200 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-100 rounded-xl outline-none text-xs text-slate-700 font-semibold appearance-none transition">
                ...
                <!-- เบอร์โทรศัพท์ติดต่อ -->
                <div>
                  <label class="block text-xs font-bold text-slate-650 mb-1.5">{{ $t('เบอร์โทรศัพท์ติดต่อ') }} <span
                      class="text-red-500">*</span></label>
                  <div class="relative">
                    <input v-model="form.finder_phoneNumber" type="tel" required :placeholder="$t('เช่น 0812345678')"
                      maxlength="10"
                      @input="form.finder_phoneNumber = form.finder_phoneNumber.replace(/\D/g, '')"
                      class="w-full pl-4 pr-10 py-2.5 bg-slate-50/50 hover:bg-slate-50/85 focus:bg-white border border-slate-200 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-100 rounded-xl outline-none text-xs text-slate-700 font-semibold transition" />
                    <div class="pointer-events-none absolute inset-y-0 right-0 flex items-center px-3.5 text-slate-400">
                ...
                <transition name="fade">
                  <div v-if="form.finder_type === 'STUDENT'" class="space-y-4">
                    <!-- เลขทะเบียนนักศึกษา -->
                    <div>
                      <label class="block text-xs font-bold text-slate-650 mb-1.5">{{ $t('เลขทะเบียนนักศึกษา') }} <span
                          class="text-red-500">*</span></label>
                      <div class="relative">
                        <input v-model="form.finder_studentId" type="text" :required="form.finder_type === 'STUDENT'"
                          :placeholder="$t('เช่น 64010123456')"
                          class="w-full pl-4 pr-10 py-2.5 bg-slate-50/50 hover:bg-slate-50/85 focus:bg-white border border-slate-200 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-100 rounded-xl outline-none text-xs text-slate-700 font-semibold transition" />
                        <div
                          class="pointer-events-none absolute inset-y-0 right-0 flex items-center px-3.5 text-slate-400">
                    ...
                    <!-- อีเมลมหาวิทยาลัย -->
                    <div>
                      <label class="block text-xs font-bold text-slate-650 mb-1.5">{{ $t('อีเมลมหาวิทยาลัย') }} <span
                          class="text-red-500">*</span></label>
                      <div class="relative">
                        <input v-model="form.finder_universityEmail" type="email"
                          :required="form.finder_type === 'STUDENT'" :placeholder="$t('เช่น student@university.ac.th')"
                          class="w-full pl-4 pr-10 py-2.5 bg-slate-50/50 hover:bg-slate-50/85 focus:bg-white border border-slate-200 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-100 rounded-xl outline-none text-xs text-slate-700 font-semibold transition" />
                        <div
                          class="pointer-events-none absolute inset-y-0 right-0 flex items-center px-3.5 text-slate-400">
              ...
              <div class="space-y-4">
                <div class="flex items-center gap-2 mb-4">
                  ...
                  <h3 class="text-xs font-extrabold text-blue-600 uppercase tracking-wider">{{ $t('ข้อมูลการจัดเก็บ') }}
                  </h3>
                </div>
                ...
                <div>
                  ...
                  <!-- Drag and Drop Zone -->
                  <div
                    class="border-2 border-dashed border-indigo-200 hover:border-indigo-400 rounded-2xl p-6 bg-slate-50/50 hover:bg-slate-50 transition flex flex-col items-center justify-center text-center cursor-pointer relative group"
                    ...
                    <div v-if="!previewUrl" class="flex flex-col items-center">
                      <div
                        class="w-12 h-12 rounded-full bg-indigo-50 flex items-center justify-center text-indigo-500 mb-3 group-hover:scale-110 transition duration-200">
                      ...
                      <span class="text-[9px] text-slate-400 mt-1">{{ $t('รองรับไฟล์ JPG, JPEG, PNG (ขนาดไม่เกิน 10MB)')
                        }}</span>
                      <button type="button"
                        class="mt-4 px-4 py-1.5 border border-slate-200 hover:border-slate-300 rounded-lg text-[10px] font-bold text-indigo-650 bg-white hover:bg-indigo-50/30 shadow-sm transition flex items-center gap-1.5">
                        <font-awesome :icon="['fas', 'file-image']" class="text-[10px] text-indigo-400" /> {{
                        $t('เลือกไฟล์') }}
                      </button>
                    </div>
                    <div v-else class="w-full flex flex-col items-center">
                      <div
                        class="relative w-full aspect-video rounded-xl overflow-hidden border border-slate-200 bg-black/5 flex items-center justify-center">
                        <img :src="previewUrl" class="max-h-full max-w-full object-contain" />
                        <button type="button" @click.stop="removeImage"
                          class="absolute top-2 right-2 w-6 h-6 rounded-full bg-slate-900/60 hover:bg-slate-900 text-white flex items-center justify-center text-xs transition">
                          &times;
                        </button>
                      </div>
                      <span class="text-[10px] text-slate-500 mt-2 font-semibold truncate max-w-[200px]">{{
                        imageFile?.name }}</span>
                    </div>
                  </div>
                </div>
                ...
                <div>
                  <label class="block text-xs font-bold text-slate-650 mb-1.5">
                    ...
                    <span class="text-[10px] text-slate-400 font-normal ml-1">{{
                      $t('(เดือนจากวันที่พบจะถูกเลือกอัตโนมัติ)') }}</span>
                  </label>
                  ...
                  <div class="grid grid-cols-2 gap-2 mb-2">
                    ...
                    <div class="relative">
                      <select v-model="form.locker_number"
                        ...
                        <option v-for="locker in allLockers" :key="locker.num" :value="locker.num">{{ locker.name }}
                        </option>
                      </select>
                      <div
                  ...
                  <div class="flex items-center gap-2 min-h-[22px]">
                    <template v-if="form.locker_number">
                      ...
                      <span
                        class="bg-indigo-50 border border-indigo-100 text-indigo-700 font-mono font-bold text-xs px-2.5 py-0.5 rounded-lg">
                ...
                <div
                  class="bg-blue-50/60 border border-blue-100 rounded-2xl p-4 flex gap-3 text-blue-700 mt-4 shadow-sm shadow-blue-100/10">
                  ...
                  <div class="flex flex-col gap-0.5">
                    ...
                    <p class="text-[10px] text-blue-650 leading-relaxed font-semibold">{{
                      $t('ข้อมูลทั้งหมดจะถูกนำไปใช้เพื่อการติดตามสิ่งของเท่านั้น และจะไม่เปิดเผยต่อบุคคลภายนอก') }}</p>
                  </div>
                </div>
