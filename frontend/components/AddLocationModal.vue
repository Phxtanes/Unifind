      ...
      <div class="bg-white rounded-3xl max-w-md w-full overflow-hidden shadow-2xl border border-slate-100 animate-scale-up flex flex-col">
        ...
        <div class="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-white shrink-0 select-none">
          <div class="flex items-center gap-3">
            ...
            <div>
              <h3 class="text-sm font-black text-slate-800 tracking-tight">{{ $t('เพิ่มสถานที่ใหม่') }}</h3>
              <p class="text-[10px] text-slate-400 font-semibold">{{ $t('สร้างสถานที่เพื่อระบุตำแหน่งของสิ่งของ') }}</p>
            </div>
          </div>
        ...
        <form @submit.prevent="handleSubmit" class="p-6 space-y-4">
          <!-- ชื่อสถานที่ -->
          <div>
            <label class="block text-xs font-bold text-slate-650 mb-1.5">{{ $t('ชื่อสถานที่ / ห้อง') }} <span class="text-red-500">*</span></label>
            <div class="relative">
              <input v-model="form.location_name" type="text" required :placeholder="$t('เช่น ห้อง 302, ลานกิจกรรม')" 
          ...
          <!-- อาคาร -->
          <div>
            <label class="block text-xs font-bold text-slate-650 mb-1.5">{{ $t('อาคาร / ตึก (ถ้ามี)') }}</label>
            <div class="relative">
              <select v-model="form.building_id" class="w-full pl-4 pr-10 py-2.5 bg-slate-50/50 hover:bg-slate-50/85 focus:bg-white border border-slate-200 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-100 rounded-xl outline-none text-xs text-slate-700 font-semibold appearance-none transition cursor-pointer">
                <option :value="null">{{ $t('ไม่ระบุอาคาร') }}</option>
          ...
          <!-- ชั้น -->
          <div>
            <label class="block text-xs font-bold text-slate-650 mb-1.5">{{ $t('ชั้น') }}</label>
            <div class="relative">
              <input v-model="form.floor" type="number" :placeholder="$t('เช่น 1, 3')" 
                class="w-full pl-4 pr-10 py-2.5 bg-slate-50/50 hover:bg-slate-50/85 focus:bg-white border border-slate-200 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-100 rounded-xl outline-none text-xs text-slate-700 font-semibold transition" />
              <div class="pointer-events-none absolute inset-y-0 right-0 flex items-center px-3.5 text-slate-400">
          ...
          <div>
            ...
            <textarea v-model="form.description" rows="2" :placeholder="$t('ระบุจุดสังเกต หรือรายละเอียดของสถานที่...')"
