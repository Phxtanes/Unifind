<template>
  <transition name="modal">
    <div v-if="show && item"
      class="fixed inset-0 z-50 overflow-y-auto bg-slate-900/60 flex items-center justify-center transition-all font-sans"
      @click.self="$emit('close')">
      <div :class="[
        (item.status === 'claimed' || item.status === 'returned') ? 'max-w-7xl' : 'max-w-5xl'
      ]"
        class="bg-white rounded-3xl shadow-2xl overflow-hidden w-full max-h-[calc(100vh-2rem)] sm:max-h-[calc(100vh-4rem)] flex flex-col border border-slate-100 modal-card transition-all duration-300">
        ...
        <div class="flex flex-col md:flex-row flex-1 overflow-y-auto">
          ...
          <div class="w-full md:w-[38%] bg-slate-100 relative min-h-[350px] md:min-h-[auto] shrink-0">
            ...
            <div class="absolute top-4 left-4 flex gap-2 z-10">
              <span
                class="px-2.5 py-1.5 text-[10px] font-bold rounded-lg shadow-sm bg-white/95 text-slate-800 border border-slate-200/50 flex items-center gap-1">
            ...
            <button @click="zoomImage"
              class="absolute top-4 right-4 w-9 h-9 bg-white/95 hover:bg-white rounded-lg shadow-sm border border-slate-200/40 flex items-center justify-center text-slate-700 transition duration-150 z-10">
              <font-awesome :icon="['fas', 'magnifying-glass-plus']" class="text-xs" />
            </button>
          </div>

          <!-- Right side: Details and information -->
          <div class="w-full md:w-[62%] p-6 flex flex-col justify-between bg-white md:border-l border-slate-100">

            <!-- Case A: Item is Claimed/Returned (Two Column Sub-Layout for Details + Claim Info) -->
            <div v-if="item.status === 'claimed' || item.status === 'returned'"
              class="grid grid-cols-1 lg:grid-cols-12 gap-5 flex-1">
              <!-- Left panel: Details -->
              <div class="lg:col-span-7 space-y-4">
                ...
                <div class="flex justify-between items-start">
                  <div class="space-y-2.5">
                    ...
                    <div class="flex flex-wrap gap-2">
                      <span
                        class="inline-flex items-center gap-1.5 px-3 py-1 bg-indigo-50/60 text-indigo-750 rounded-full text-[11px] font-extrabold border border-indigo-100/50">
                        <font-awesome :icon="['fas', 'circle-down']" class="text-indigo-500 text-[10px]" />
                        {{ translateCategory(item.category) }}
                      </span>
                      <span
                        class="inline-flex items-center gap-1.5 px-3 py-1 bg-emerald-50/60 text-emerald-750 border border-emerald-100/50 rounded-full text-[11px] font-extrabold border">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <!-- Location Card -->
                  <div
                    class="bg-white border border-slate-100 rounded-2xl p-3 flex items-center gap-2.5 shadow-sm hover:shadow transition duration-150">
                    <div class="w-9 h-9 rounded-xl bg-rose-50 flex items-center justify-center shrink-0">
                      <font-awesome :icon="['fas', 'location-dot']" class="text-rose-500 text-xs" />
                    </div>
                    <div class="min-w-0">
                      <p class="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-0.5">{{
                        $t('สถานที่ที่พบ') }}</p>
                      <p class="text-xs font-bold text-slate-800 truncate" :title="item.place">{{ item.place || '-' }}
                      </p>
                    </div>
                  </div>
                  ...
                  <div
                    ...
                    <div class="w-9 h-9 rounded-xl bg-indigo-50 flex items-center justify-center shrink-0">
                      <font-awesome :icon="['fas', 'calendar-days']" class="text-indigo-500 text-xs" />
                    </div>
                    <div class="min-w-0">
                      <p class="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-0.5">{{
                        $t('วันที่และเวลาบันทึกของ') }}</p>
                      <p class="text-xs font-bold text-slate-800 truncate" :title="formatFullDate(item.date)">{{
                        formatFullDate(item.date) }}</p>
                    </div>
                  </div>
                  ...
                  <div
                    ...
                    <div class="w-9 h-9 rounded-xl bg-amber-50 flex items-center justify-center shrink-0">
                      <font-awesome :icon="['fas', 'box']" class="text-amber-500 text-xs" />
                    </div>
                    <div class="min-w-0">
                      <p class="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-0.5">{{
                        $t('ตู้ล็อกเกอร์จัดเก็บ') }}</p>
                      <p class="text-xs font-bold text-slate-800 truncate" :title="item.locker">{{ item.locker || '-' }}
                      </p>
                    </div>
                  </div>
                  ...
                  <div
                    ...
                    <div class="w-9 h-9 rounded-xl bg-blue-50 flex items-center justify-center shrink-0">
                      <font-awesome :icon="['fas', 'user']" class="text-blue-500 text-xs" />
                    </div>
                    <div class="min-w-0">
                      <p class="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-0.5">{{
                        $t('เจ้าหน้าที่ผู้บันทึก') }}</p>
                      <p class="text-xs font-bold text-slate-800 truncate" :title="item.staffName">{{ item.staffName ||
                        'Admin' }}</p>
                    </div>
                  </div>
                </div>
                ...
                <div class="bg-white border border-slate-100 rounded-2xl p-3 shadow-sm">
                  <div class="flex items-center gap-2 mb-2 text-slate-700">
                    <font-awesome :icon="['fas', 'file-lines']" class="text-slate-400 text-xs" />
                    <p class="text-[10px] font-bold uppercase tracking-wider text-slate-555">{{
                      $t('รายละเอียดเพิ่มเติม') }}</p>
                  </div>
                  <div class="bg-slate-50/70 border border-slate-100 rounded-xl p-2.5 min-h-[50px]">
                    <p class="text-xs text-slate-600 whitespace-pre-wrap leading-relaxed">{{
                      formatDescription(item.description) }}</p>
                  </div>
                </div>

                <!-- Finder/Reporter Info (New Separate Box) -->
                <div v-if="(item.type === 'found' && item.finderName) || (item.type === 'lost' && item.reporterName)"
                  class="bg-white border border-slate-100 rounded-2xl p-3 shadow-sm">
                  <div
                    class="flex items-center justify-between p-2.5 bg-indigo-50/60 border border-indigo-100/70 rounded-xl">
                    <div class="flex items-center gap-2 text-indigo-800">
                      <font-awesome :icon="['fas', 'user']" class="text-indigo-500 text-xs" />
                      <span class="text-xs font-bold">
                        {{ item.type === 'found' ? $t('ผู้พบเจอ / ผู้นำส่ง') : $t('ผู้แจ้ง / ผู้ทำหาย') }}
                      </span>
                    </div>
                    <div class="text-right">
                      <span class="text-xs font-extrabold text-indigo-950">
                        {{ item.type === 'found' ? item.finderName : item.reporterName }}
                      </span>
                      <span v-if="item.type === 'found' ? item.finderPhone : item.reporterPhone"
                        class="block text-[10px] text-indigo-600/80 font-mono mt-0.5">
                        {{ item.type === 'found' ? item.finderPhone : item.reporterPhone }}
                      </span>
                    </div>
                  </div>
                </div>
              ...
              <div
                class="lg:col-span-5 bg-emerald-50 border border-emerald-200/60 p-6 rounded-3xl space-y-4 self-stretch shadow-sm shadow-emerald-100/10 flex flex-col justify-start">
                ...
                <div class="divide-y divide-emerald-200/50 text-xs flex-1">
                  ...
                  <div class="flex justify-between py-2.5">
                    ...
                    <span class="font-extrabold text-emerald-950">
                      {{ item.claimerType === 'STUDENT' ? $t('นักศึกษา') : item.claimerType === 'STAFF' ? $t('บุคลากร')
                        : item.claimerType === 'MAID' ? $t('แม่บ้าน') : item.claimerType === 'SECURITY' ?
                          $t('เจ้าหน้าที่รักษาความปลอดภัย') : $t('บุคคลภายนอก') }}
                    </span>
                  </div>
                  ...
                  <div v-if="item.remark" class="flex flex-col py-2.5 gap-1.5">
                    ...
                    <span
                      class="font-semibold text-emerald-900 bg-white/70 p-2.5 rounded-xl border border-emerald-200/50 leading-relaxed">{{
                        item.remark }}</span>
                  </div>
                </div>
              </div>
            ...
            <div v-else class="space-y-4 flex-1">
                ...
                <div class="space-y-2.5">
                  ...
                  <div class="flex flex-wrap gap-2">
                    ...
                    <span :class="{
                      ...
                      <font-awesome :icon="item.status === 'lost' ? ['fas', 'circle-xmark'] : ['fas', 'circle-check']"
                        :class="item.status === 'lost' ? 'text-rose-500' : 'text-emerald-500'" class="text-[10px]" />
              ...
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div
                  ...
                  <div class="min-w-0">
                    <p class="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-0.5">{{ $t('สถานที่')
                    }}{{ item.type === 'lost' ? $t('หาย') : $t('ที่พบ') }}</p>
              ...
              <div class="bg-white border border-slate-100 rounded-2xl p-3 shadow-sm">
                <div class="flex items-center gap-2 mb-2 text-slate-700">
                  <font-awesome :icon="['fas', 'file-lines']" class="text-slate-400 text-xs" />
                  <p class="text-[10px] font-bold uppercase tracking-wider text-slate-555">{{ $t('รายละเอียดเพิ่มเติม')
                  }}</p>
                </div>
                <div class="bg-slate-50/70 border border-slate-100 rounded-xl p-2.5 min-h-[50px]">
                  <p class="text-xs text-slate-600 whitespace-pre-wrap leading-relaxed">{{
        ...
        <div class="bg-slate-50 px-8 py-4 border-t border-slate-100 flex items-center justify-between select-none">
          <button @click="$emit('close')"
            class="flex items-center gap-2 px-4 py-2 bg-white border border-slate-200 hover:bg-slate-50 text-slate-550 hover:text-slate-700 text-xs font-extrabold rounded-xl transition duration-150 shadow-sm">
            <font-awesome :icon="['fas', 'arrow-left']" />
            {{ $t('ย้อนกลับ') }}
          </button>
          <button v-if="item.status !== 'claimed' && item.status !== 'returned'" @click="$emit('edit', item)"
            class="flex items-center gap-2 px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-extrabold rounded-xl transition duration-150 shadow-md shadow-indigo-600/10">
