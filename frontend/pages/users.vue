<template>
  <div class="space-y-6 font-sans">
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
      ...
      <!-- Add User Button -->
      <button v-if="isAdminUser" @click="showAddModal = true"
    ...
    <div v-if="!isAdminUser" class="bg-rose-50 border border-rose-200 rounded-2xl p-6 text-center text-rose-800">
      <div
        class="w-12 h-12 rounded-full bg-rose-100 flex items-center justify-center text-lg mx-auto shadow-sm text-rose-600 mb-3">
      ...
      <p class="text-xs text-rose-600 mt-1">{{ $t('เฉพาะผู้ดูแลระบบ (Admin) เท่านั้นที่สามารถจัดการบัญชีผู้ใช้ได้') }}
      </p>
    </div>
    ...
    <div v-else class="space-y-6 animate-fade-in-up">
      ...
      <div
        class="bg-white rounded-xl py-3 px-4 border border-slate-200/80 shadow-sm flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <!-- Tabs selection -->
        <div class="flex bg-slate-100 p-1 rounded-xl shrink-0 self-start sm:self-auto">
          <button @click="activeTab = 'staff'" :class="[
            activeTab === 'staff'
        ...
        <div class="relative w-full max-w-xs">
          ...
          <input v-model="searchQuery" type="text" :placeholder="$t('ค้นหาผู้ใช้...')"
      ...
      <div v-if="activeTab === 'staff'"
        class="bg-white pt-0 px-6 pb-6 rounded-2xl border border-slate-200/80 shadow-sm flex flex-col justify-between min-h-[480px]">
        <div class="pt-6 mb-4">
        ...
        <div class="overflow-x-auto -mx-6 flex-1">
          <table class="w-full text-left border-collapse min-w-[700px]">
            <thead>
              <tr
                class="border-b border-slate-200 text-[10px] font-bold text-slate-500 uppercase tracking-widest bg-slate-50/70">
            ...
            <tbody class="divide-y divide-slate-100 text-xs text-slate-700">
              <tr v-for="user in filteredStaff" :key="user.user_id"
                class="hover:bg-indigo-50/30 transition-all duration-150">
                ...
                <td class="py-4 px-6 font-medium text-slate-900 flex items-center gap-2">
                  <span
                    class="w-8 h-8 rounded-full bg-slate-100 border border-slate-200 flex items-center justify-center font-medium text-slate-600">
                ...
                <td class="py-4 px-6 text-slate-455 font-medium font-mono">{{ formatFullDate(user.created_at ||
                  user.createdAt) }}</td>
                <td class="py-4 px-6">
                  <div class="flex items-center justify-center gap-2">
                    ...
                    <button v-if="user.status === 'Active' || user.status === 'active'"
                    ...
                    <button v-else @click="activateUserAccount(user.user_id)"
                    ...
                    <button @click="deleteUserAccount(user.user_id)"
              ...
              <tr v-if="filteredStaff.length === 0">
                <td colspan="6" class="py-20 text-center text-slate-400 font-medium">{{
                  $t('ไม่พบบัญชีเจ้าหน้าที่ในระบบ') }}</td>
              </tr>
            </tbody>
          </table>
        </div>
      ...
      <div v-else
        ...
        <div class="overflow-x-auto -mx-6 flex-1">
          <table class="w-full text-left border-collapse min-w-[700px]">
            ...
            <tbody class="divide-y divide-slate-100 text-xs text-slate-700">
              <tr v-for="user in filteredPending" :key="user.user_id"
                class="hover:bg-indigo-50/30 transition-all duration-150">
                <td class="py-4 px-6 font-bold text-slate-900 flex items-center gap-2">
                  <span
                    class="w-8 h-8 rounded-full bg-slate-100 border border-slate-200 flex items-center justify-center font-bold text-slate-600">
                    {{ user.username.slice(0, 2).toUpperCase() }}
                  </span>
                  {{ user.username }}
                </td>
                ...
                <td class="py-4 px-6">
                  <span
                    class="bg-amber-50 text-amber-700 border-amber-100 px-2 py-0.5 rounded text-[10px] font-bold border uppercase">
                    MEMBER -> STAFF
                  </span>
                </td>
                <td class="py-4 px-6">
                  <span
                    class="inline-flex items-center gap-1.5 px-2.5 py-0.5 bg-blue-50 text-blue-700 border border-blue-100 text-[10px] font-bold rounded-full uppercase">
                ...
                <td class="py-4 px-6 text-center">
                  <div class="flex items-center justify-center gap-2">
                    <button @click="approveRequest(user.user_id)"
                    ...
                    <button @click="rejectRequest(user.user_id)"
              ...
              <tr v-if="filteredPending.length === 0">
                <td colspan="6" class="py-20 text-center text-slate-400 font-medium">{{
                  $t('ไม่มีคำขอที่รอการอนุมัติสิทธิ์เข้าใช้งาน') }}</td>
              </tr>
            </tbody>
          </table>
        </div>
    ...
    <div v-if="showAddModal"
      class="fixed inset-0 z-50 overflow-y-auto bg-slate-900/60 flex items-center justify-center p-4"
      @click.self="showAddModal = false">
      <div
        class="bg-white rounded-2xl shadow-2xl border border-slate-200/80 overflow-hidden max-w-md w-full animate-fade-in-up font-sans">
        <div class="bg-slate-900 px-6 py-4 flex justify-between items-center text-white border-b border-slate-800">
          ...
          <button @click="showAddModal = false"
            class="text-slate-400 hover:text-white text-xl font-semibold outline-none">&times;</button>
        </div>
        ...
        <form @submit.prevent="createNewUser" class="p-6 space-y-4">
          ...
          <div>
            ...
            <input type="text" v-model="newUserForm.nickname" :placeholder="$t('กรอกชื่อเล่น...')"
          ...
          <div>
            ...
            <input type="text" v-model="newUserForm.username" required :placeholder="$t('กรอกชื่อผู้ใช้งาน...')"
              class="w-full px-3.5 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-medium text-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition" />
          </div>

          <div>
            <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">{{ $t('อีเมลติดต่อ (Email) *') }}</label>
            <input type="email" v-model="newUserForm.email" required placeholder="example@unifind.local..."
              class="w-full px-3.5 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-medium text-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition" />
          </div>

          <div>
            <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">{{ $t('รหัสผ่าน (Password) *') }}</label>
            <input type="password" v-model="newUserForm.password" required :placeholder="$t('กรอกรหัสผ่านเริ่มต้น...')"
              class="w-full px-3.5 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-medium text-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition" />
          </div>
    ...
    <div v-if="showEditModal"
      ...
      <div
        ...
        <form @submit.prevent="updateUserAccount" class="p-6 space-y-4">
          <div>
            <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">{{ $t('ชื่อเล่น (Nickname)') }}</label>
            <input type="text" v-model="editUserForm.nickname" :placeholder="$t('กรอกชื่อเล่น...')"
              class="w-full px-3.5 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-medium text-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition" />
          </div>

          <div>
            <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">{{ $t('ชื่อผู้ใช้งาน (Username) *') }}</label>
            <input type="text" v-model="editUserForm.username" required :placeholder="$t('กรอกชื่อผู้ใช้งาน...')"
              class="w-full px-3.5 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-medium text-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition" />
          </div>

          <div>
            <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">{{ $t('อีเมลติดต่อ (Email) *') }}</label>
            <input type="email" v-model="editUserForm.email" required placeholder="example@unifind.local..."
              class="w-full px-3.5 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-medium text-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition" />
          </div>
          ...
          <div>
            ...
            <div v-if="isEditingSelf">
              <input type="password" v-model="editUserForm.password"
                :placeholder="$t('กรอกรหัสผ่านใหม่... (เว้นว่างไว้หากไม่ต้องการเปลี่ยน)')"
                class="w-full px-3.5 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-medium text-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition" />
            </div>
            <div v-else class="space-y-1">
              <input type="password" disabled :placeholder="$t('ไม่อนุญาตให้เปลี่ยนรหัสผ่านของผู้อื่น')"
          ...
          <div class="grid grid-cols-2 gap-4">
            ...
            <div>
              <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">{{ $t('สถานะ')
                }}</label>
