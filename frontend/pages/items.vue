<template>
  <div class="space-y-6">
    ...
    <div>
      ...
      <p class="text-xs text-slate-400 mt-0.5">รายการสิ่งของทุกสถานะในระบบ UTCC Unifind</p>
    </div>
    ...
    <div class="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm flex flex-col md:flex-row md:items-center md:justify-between gap-4">
      ...
      <div class="flex items-center gap-3">
        <span class="text-xs font-bold text-slate-400 uppercase tracking-wider">หมวดหมู่ปัจจุบัน:</span>
        <span class="bg-indigo-50 text-indigo-700 text-xs font-bold px-3 py-1.5 rounded-lg border border-indigo-100 uppercase">ทั้งหมด</span>
      </div>
    </div>
    ...
    <div class="bg-white p-6 rounded-2xl border border-slate-200/80 shadow-sm flex flex-col justify-between min-h-[480px]">
        ...
        <table class="w-full text-left border-collapse">
          <thead>
            <tr class="border-b border-slate-100 text-[10px] font-extrabold text-slate-450 uppercase tracking-wider bg-slate-50/50">
              ...
              <th class="py-3 px-4">สถานะสิ่งของ</th>
          ...
          <tbody class="divide-y divide-slate-50">
            <tr v-for="item in paginatedItems" :key="item.id" class="hover:bg-slate-50/40 text-xs transition duration-100">
              <td class="py-3 px-4 flex items-center gap-3">
                ...
                <div v-else class="w-10 h-10 rounded-lg bg-slate-50 flex items-center justify-center text-slate-400 border border-slate-150 font-bold">📷</div>
                <div>
              ...
              <td class="py-3 px-4 text-slate-650 font-medium">{{ translateCategory(item.category) }}</td>
              <td class="py-3 px-4">
                <span :class="{
                ...
                }" class="px-2.5 py-0.5 text-[9px] font-bold rounded border uppercase">
              ...
              <td class="py-3 px-4">
                <div class="flex items-center justify-center gap-2">
                  ...
                  <button @click="deleteItem(item.id)" class="text-slate-400 hover:text-rose-600 p-1.5 rounded-lg border border-transparent hover:border-slate-100 hover:bg-slate-50 transition">
                    🗑️
                  </button>
                </div>
              </td>
            ...
            <tr v-if="paginatedItems.length === 0">
              <td colspan="7" class="text-center py-20 text-slate-450">
                <p class="text-lg">🔎</p>
                <p class="text-xs mt-2 font-medium">ไม่พบรายการข้อมูลบันทึกสิ่งของในระบบ</p>
              </td>
            </tr>
          </tbody>
        </table>
      ...
      <div v-if="totalPages > 1" class="mt-4 flex justify-between items-center text-xs font-medium text-slate-500 pt-3 border-t border-slate-50">
        <span>แสดงหน้า {{ currentPage }} จากทั้งหมด {{ totalPages }} หน้า (ทั้งหมด {{ filteredItems.length }} รายการ)</span>
