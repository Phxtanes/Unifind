<template>
  <div class="space-y-6">
      ...
      <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p class="text-xs text-slate-500 font-medium mt-1">
            {{ $t('เปรียบเทียบสิ่งของที่แจ้งหายและของที่พบในระบบด้วย Gemini AI วิเคราะห์ความน่าจะเป็นในการจับคู่แบบสองทิศทาง') }}</p>
        </div>
    ...
    <div class="grid grid-cols-1 xl:grid-cols-12 gap-6 items-start">
      ...
      <div
        class="xl:col-span-4 bg-white rounded-2xl border border-slate-200/80 shadow-sm overflow-hidden flex flex-col h-[680px]">
        ...
        <div class="flex-1 overflow-y-auto divide-y divide-slate-100 p-2">
          <div v-for="item in filteredLostItems" :key="item.id" @click="selectLostItem(item)" :class="[
            ...
            <img v-if="getItemImageSrc(item)" :src="getItemImageSrc(item)"
              class="w-12 h-12 rounded-lg object-cover border border-slate-200 shadow-sm shrink-0" />
            <div v-else
              class="w-12 h-12 rounded-lg bg-rose-50 border border-rose-100 flex items-center justify-center text-rose-400 shrink-0 shadow-sm">
            ...
            <div class="min-w-0 flex-1 flex flex-col justify-between">
              <div>
                ...
                <p class="text-[10px] text-slate-500 font-medium truncate mt-0.5">{{ translateCategory(item.category) }}
                </p>
              </div>
      ...
      <div
        ...
        <div class="flex-1 overflow-y-auto divide-y divide-slate-100 p-2">
          ...
          <div v-if="!selectedLost" class="text-center py-32 px-6 text-slate-400">
            <div
              class="w-12 h-12 rounded-full bg-slate-50 flex items-center justify-center mx-auto text-slate-400 border border-slate-150 mb-3 shadow-inner">
              <font-awesome :icon="['fas', 'arrow-left']" />
            </div>
            <h4 class="text-xs font-bold text-slate-600">{{ $t('กรุณาเลือกรายการของหายก่อน') }}</h4>
            <p class="text-[10px] mt-1">{{
              $t('ระบบจะแสดงรายชื่อของพบที่อยู่ในหมวดหมู่เดียวกันขึ้นมาเปรียบเทียบแบบอัตโนมัติ') }}</p>
          </div>
          ...
          <div v-else v-for="item in filteredFoundItems" :key="item.id" @click="selectFoundItem(item)" :class="[
            ...
            <img v-if="getItemImageSrc(item)" :src="getItemImageSrc(item)"
              class="w-12 h-12 rounded-lg object-cover border border-slate-200 shadow-sm shrink-0" />
            <div v-else
              class="w-12 h-12 rounded-lg bg-emerald-50 border border-emerald-100 flex items-center justify-center text-emerald-400 shrink-0 shadow-sm">
      ...
      <div class="xl:col-span-4 space-y-6">
        ...
        <div
          class="bg-white rounded-2xl border border-slate-200/80 shadow-sm p-5 min-h-[300px] flex flex-col justify-between">
          ...
          <div class="space-y-4 flex-1">
            ...
            <div class="bg-slate-50 border border-slate-200 rounded-xl p-3.5 flex items-center gap-3">
              <div
                class="w-10 h-10 rounded-lg bg-rose-50 flex items-center justify-center text-rose-550 border border-rose-100 text-xs shrink-0 shadow-inner">
                <font-awesome :icon="['fas', 'briefcase']" />
              </div>
            ...
            <div class="bg-slate-50 border border-slate-200 rounded-xl p-3.5 flex items-center gap-3">
              <div
                class="w-10 h-10 rounded-lg bg-emerald-50 flex items-center justify-center text-emerald-650 border border-emerald-100 text-xs shrink-0 shadow-inner">
                <font-awesome :icon="['fas', 'box-open']" />
              </div>
              <div class="min-w-0 flex-1">
                <p class="text-[9px] font-bold text-emerald-700 uppercase tracking-wide">{{ $t('ฝั่งคลังสิ่งของพบเจอ')
                }}</p>
                <h4 class="font-extrabold text-slate-800 text-[11px] truncate mt-0.5">
          ...
          <!-- Trigger AI button -->
          <div class="mt-6">
            <button @click="runAIPairMatching" :disabled="!selectedLost || !selectedFound || isAnalyzing"
              ...
              <font-awesome :icon="['fas', isAnalyzing ? 'spinner' : 'robot']"
        ...
        <div v-if="matchResult || isAnalyzing"
          class="bg-white rounded-2xl border border-slate-200/80 shadow-sm p-6 flex flex-col items-center justify-center min-h-[250px] animate-fade-in">
          <!-- Scanning state -->
          <div v-if="isAnalyzing" class="text-center py-6 space-y-3">
            <div
              class="w-14 h-14 rounded-2xl bg-indigo-50 border border-indigo-100 flex items-center justify-center text-indigo-600 text-xl mx-auto shadow-sm animate-pulse">
              <font-awesome :icon="['fas', 'microchip']" class="animate-spin" />
            </div>
            <h4 class="text-xs font-bold text-slate-700">{{ $t('กำลังอ่านข้อมูลประมวลผล...') }}</h4>
            <p class="text-[10px] text-slate-400">{{ $t('Gemini AI กำลังวิเคราะห์ชื่อ ตำหนิ สถานที่และวันเวลาสูญหาย') }}
            </p>
          </div>
          ...
          <div v-else-if="matchResult" class="w-full space-y-4">
            ...
            <div class="bg-slate-50 border border-slate-200 rounded-xl p-4">
              <span class="text-[9px] font-bold text-slate-450 uppercase tracking-wider">{{ $t('ข้อวินิจฉัยจาก AI:')
              }}</span>
            ...
            <div class="flex gap-2 pt-2" v-if="matchResult.matched">
              <button @click="processClaimSuccess"
...
const filteredFoundItems = computed(() => {
  ...
  // ค้นหาข้อมูลตามคำค้นหาที่พิมพ์ในช่องค้นหาฝั่งของที่พบ
...
// ฟังก์ชันเลือกรายการสิ่งของฝั่งของหาย (Lost Item)
...
// ฟังก์ชันเลือกรายการสิ่งของฝั่งของที่พบ (Found Item)
const selectFoundItem = (item: any) => {
  selectedFound.value = item
  matchResult.value = null
}

// ฟังก์ชันเรียก API ให้ Gemini AI ตรวจสอบเปรียบเทียบความน่าจะเป็นในการเข้าคู่กันระหว่างสองสิ่งของ
const runAIPairMatching = async () => {
  ...
  try {
    ...
    // ส่ง ID ของสิ่งของทั้งสองรายการไปวิเคราะห์คู่กันที่หลังบ้าน
    ...
    matchResult.value = response.data // บันทึกผลลัพธ์การจับคู่จาก AI ลงใน state
  } catch (error) {
...
// ฟังก์ชันเมื่อกดปุ่มทำเรื่องคืนของ: ส่งผู้ใช้ไปยังหน้าพบของ (/found) เพื่อกรอกฟอร์มพร้อมรายละเอียดการรับคืน
const processClaimSuccess = () => {
  if (!selectedFound.value) return
  router.push(`/found?returnItemId=${selectedFound.value.id}`)
}
</script>
