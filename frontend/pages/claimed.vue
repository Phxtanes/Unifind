<template>
  <div class="space-y-6">
    ...
    <div>
      <h2 class="text-lg font-bold text-slate-900">🔄 รายการคืนแล้ว</h2>
      <p class="text-xs text-slate-400 mt-0.5">สิ่งของที่ส่งคืนเจ้าของเรียบร้อยแล้ว</p>
    </div>
    ...
    <div class="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm flex flex-col md:flex-row md:items-center md:justify-between gap-4">
      ...
      <div class="flex items-center gap-3">
        ...
        <span class="bg-indigo-50 text-indigo-700 text-xs font-bold px-3 py-1.5 rounded-lg border border-indigo-100 uppercase">คืนแล้ว (Claimed)</span>
      </div>
    </div>
          ...
          <tbody class="divide-y divide-slate-50">
            <tr v-for="item in paginatedItems" :key="item.id" class="hover:bg-slate-50/40 text-xs transition duration-100">
              <td class="py-3 px-4 flex items-center gap-3">
                ...
                <div v-else class="w-10 h-10 rounded-lg bg-slate-50 flex items-center justify-center text-slate-400 border border-slate-100">🔄</div>
                <div>
              ...
              <td class="py-3 px-4">
                <span class="px-2.5 py-0.5 text-[9px] font-bold rounded border uppercase bg-slate-100 text-slate-700 border-slate-200">คืนสำเร็จ</span>
              </td>
            ...
            <tr v-if="paginatedItems.length === 0">
              <td colspan="7" class="text-center py-20 text-slate-450">
                <p class="text-lg">🔄</p>
                <p class="text-xs mt-2 font-medium">ยังไม่มีรายการที่คืนเจ้าของ</p>
              </td>
            </tr>
          </tbody>
        </table>
