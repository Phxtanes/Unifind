...
exports.createClaim = ({
  ...
  const lastId =
    claims.length > 0 ? Math.max(...claims.map((c) => c.claim_id)) : 0;
  const newClaim = {
    claim_id: lastId + 1,
...
exports.rejectOtherPendingClaims = (found_item_id, approved_claim_id) => {
  ...
  claims.forEach((c) => {
    if (
      c.found_item_id === parseInt(found_item_id) &&
      c.claim_id !== parseInt(approved_claim_id) &&
      c.status === "pending"
    ) {
      c.status = "rejected";
      c.reject_reason =
        "สิ่งของนี้ถูกส่งคืนให้กับคำร้องอื่นที่ได้รับการอนุมัติแล้ว";
      updated = true;
    }
  });
